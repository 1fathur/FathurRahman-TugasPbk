...
import { useTodosStore } from '../stores/tudos';
...
const filteredKegiatan = computed(() => {
  if (filterAktif.value) {
    return store.daftarKegiatan.filter(kegiatan => !kegiatan.selesai);
  } else {
    return store.daftarKegiatan;
  }
});

const tambahKegiatan = () => {
  if (namaKegiatan.value.trim() !== '') {
    store.updateDaftarKegiatan([...store.daftarKegiatan, { nama: namaKegiatan.value, selesai: false }]);
    namaKegiatan.value = '';
  }
};

const tandaiSelesai = (index) => {
  const updatedKegiatan = [...store.daftarKegiatan];
  updatedKegiatan[index].selesai = true;
  store.updateDaftarKegiatan(updatedKegiatan);
};
...
const hapusKegiatan = (index) => {
  const updatedKegiatan = store.daftarKegiatan.filter((_, i) => i !== index);
  store.updateDaftarKegiatan(updatedKegiatan);
};
